...
class SimulationParametersTran:
  ...
  # This is essentially the reading voltage for the FSST analysis
        ...
        if self.cs_bias == "perfect_curr_source": 
            self.layer1_bias_curr = 3 * 5 * input_size * 1e-6 #When I am using large networks that are difficult to bias with the nmos sources I am using DC sources with this bias current
            self.layer2_bias_curr = 3 * 5 * size_of_hidden_layer * 1e-6 #The idea is that for each synapse that is connected to the neuron they should provide 10e-6 Amps
        ...
        self.start_read_time = 5e-6 #the pulse starts at start_read time then it reaches the max value after the 1u and it stays there
        self.end_read_time = 40e-6
        ...
        self.start_discharge_time = 42e-6
        self.end_discharge_time = 45-6
        self.rise_time = 1e-6

...
def initialize_network_layers(simulation_parameters):
    ...
    # Input Layer
    input_layer = InputLayer(fc_layers[0], freq, simulation_type, which_layer=0)
    ...
    which_layer = 0
    layer1 = DenseLayer(n_of_inputs, n_of_outputs, synapse, bounds, gamma1, weight_initializer, which_layer)
    layer1.initialize_W()
    layer1.update_synapse_dict()
    layers.append(layer1)
        
    # Non-linear Layer (using 'neuron' as the neuron type)
    layer2 = NonLinearLayer(n_of_outputs, neuron, cs_bias, non_lin, which_layer)
    ...
    which_layer = 1
    layer3 = DenseLayer(n_of_inputs, n_of_outputs, synapse, bounds, gamma2, weight_initializer, which_layer)
    layer3.initialize_W()
    ...
    layer4 = OutputLayer(n_of_inputs, freq, simulation_type, nudging_mode, cs_bias, which_layer)

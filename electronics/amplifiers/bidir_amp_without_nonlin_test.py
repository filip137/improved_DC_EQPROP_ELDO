bidir_amp_without_nonlin_test = {"PARAMS":
"""
.PARAM W_NONLIN_NMOS=3u
.PARAM W_NONLIN_PMOS=6u
.PARAM L_NONLIN_PMOS=1u
.PARAM L_NONLIN_NMOS=1u
.PARAM R_NONLIN2PMOS=4.1MEG
.PARAM R_NONLIN1PMOS=700K
.PARAM R_NONLIN1NMOS=4.1MEG
.PARAM R_NONLIN2NMOS=700K
.PARAM PMOS_CCCS_CASCADE_BIAS=1.7V
...
XM0 AMP_INPUT NET019 VDD_NMOS_NONLIN VDD_NMOS_NONLIN EN5V0_BS3JU
+w=W_NONLIN_NMOS l=L_NONLIN_NMOS nfing=1 ncrsd=1 number=1 srcefirst=1
+ngcon=1 mismatch=1 po2act=-1 dvt_mdev=0 dmu_mdev=0 soa=1 lpe=0
    C8 NET019 NET023 CAP_NONLIN_INPUT
    C7 NET017 NET023 CAP_NONLIN_INPUT
    C3 AMP_INPUT NET06 CAP_VCVS_INPUT
    C0 AMP_INPUT NET17 CAP_CCCS
    XI1 NET06 NET023 NET022 VCVS_SS
    XI2 NET17 NET022 AMP_OUTPUT CCCS_SS
XM1 AMP_INPUT NET017 VDD_PMOS_NONLIN VDD_PMOS_NONLIN EP5V0_BS3JU
+w=W_NONLIN_PMOS l=L_NONLIN_PMOS nfing=1 ncrsd=1 number=1 srcefirst=1
+ngcon=1 mismatch=1 po2act=-1 dvt_mdev=0 dmu_mdev=0 soa=1 lpe=0
    V2 VDD 0 DC VDD
    V1 VDD_NMOS_NONLIN 0 DC VDD_NMOS_NONLIN
    V0 VDD_PMOS_NONLIN 0 DC VDD_PMOS_NONLIN
    R5 VDD NET019 R_NONLIN1NMOS
    R4 NET019 0 R_NONLIN2NMOS
    R3 NET017 0 R_NONLIN2PMOS
    R2 VDD NET017 R_NONLIN1PMOS
    R0 NET06 0 R_VCVS_BIAS2
    R1 VDD NET06 R_VCVS_BIAS1
.ENDS
*** End of subcircuit definition.
"""

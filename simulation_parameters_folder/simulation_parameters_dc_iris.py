...
class SimulationParametersDC_iris:
    def __init__(self, scale_factor, bias, batch_size, beta, gamma_values, output_scale, load_weights, h5_file):
        ...
        # Assign amplifier
        self.amplifier = "BiDirWithNonLin"  # allowed options below
  ...
  # This is essentially the reading voltage for the FSST analysis
        ...
        if self.cs_bias == "perfect_curr_source": 
            self.layer1_bias_curr = 3 * 5 * 5 * 1e-6  * 0#When I am using large networks that are difficult to bias with the nmos sources I am using DC sources with this bias current
            self.layer2_bias_curr = 3 * 5 * 12 * 1e-6 * 0 #The idea is that for each synapse that is connected to the neuron they should provide 10e-6 Amps
        ...
        self.bounds = {"min_conductance" : 0.1e-7,
                       "max_conductance" : 70e-5}
        ...
        if mode == "TRAIN":
            ...
            base_models = "/home/filip/simulations/testing_models"
        ...
        # Dataset parameters
        self.dataset = "iris_simulation"
        self.n_of_epochs = 300

...
class SimulationParametersFSST_iris:
    def __init__(self, scale_factor, bias, batch_size, beta, gamma_values, output_scale, load_weights, h5_file):
        ...
        self.network_size = [9, 15, 6]  # [input, hidden, output]
        self.freq = "5MEG"
        self.neuron = "amp_ss"  # amp_ss or perfect_amp
        self.amplifier = "BiDirWithOutNonLin" # "BiDirWithNonLin" or "BiDirWithOutNonLin" or OldBiDirAmp or ThreeTerminalBiDirAmp
  ...
  # This is essentially the reading voltage for the FSST analysis
        ...
        hidden_layer_size = self.network_size[1]
        if self.cs_bias == "perfect_curr_source": 
            self.layer1_bias_curr = 5 * 9 * 1e-6  #When I am using large networks that are difficult to bias with the nmos sources I am using DC sources with this bias current
            self.layer2_bias_curr = 5 * hidden_layer_size * 1e-6 #The idea is that for each synapse that is connected to the neuron they should provide 10e-6 Amps
        ...
        # Dataset parameters
        self.dataset = "iris_simulation" #or moons_simulation or digits_simulation
        self.n_of_epochs = 150
        ...
        self.vdc_bias1 = 2.3 #essentially the input reading voltage for the FSST

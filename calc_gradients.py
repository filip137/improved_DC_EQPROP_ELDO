...
def main():
    ...
    # for key in resistor_value_dict:
    #     resistor_value_dict[key] = np.random.uniform(10e3, 10e8)
    ...
    for vac2 in vac2_v: 
        ...
        for voltage in input_voltages:
            ...
            delta_arr = np.linspace(0.001, 0.15, 100)
            loss_grads = []
            ...
            grad1_list = []
            beta_arr = np.linspace(1e-10, 1e-7, 100)
            voltage_nudge_list = []
        ...
        plt.show()
            ...
            # Plot each column on the same figure
        plt.figure(figsize=(8, 5))
        stored_currents_arr = np.array(stored_currents)
